Given(/^I have added a discussion with the title "([^"]*)" and body "([^"]*)" and author "([^"]*)"$/) do |arg1, arg2, arg3|
  pending
end

When(/^I have visited the main "([^"]*)" page$/) do |arg|
  pending
end

Then(/^I should see the discussion post by "([^"]*)"$/) do |arg|
  pending
end

And(/^I am on the discussions home page$/) do
  pending
end

And(/^I am on the discussion page with the title "([^"]*)" and authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

When(/^I post a reply with body "([^"]*)" and authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

Then(/^I should see a reply with body "([^"]*)" and authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

When(/^I have deleted the discussion with the title "([^"]*)" authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

Then(/^I should not see the discussion post by "([^"]*)"$/) do |arg|
  pending
end

And(/^I should be redirected to the discussion homepage$/) do
  pending
end

And(/^There is a reply with body "([^"]*)" authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

And(/^I should be redirected to the discussion page for the discussion with title "([^"]*)" authored by "([^"]*)"$/) do |arg1, arg2|
  pending
end

And(/^I edit the discussion titled "([^"]*)" by "([^"]*)" with title "([^"]*)" and body "([^"]*)"$/) do |arg1, arg2, arg3, arg4|
  pending
end

Then(/^I should see the discussion post by "([^"]*)" with title "([^"]*)" and body "([^"]*)"$/) do |arg1, arg2, arg3|
  pending
end

When(/^I edit discussion reply with body "([^"]*)" authored by "([^"]*)" to body "([^"]*)"$/) do |arg1, arg2, arg3|
  pending
end
